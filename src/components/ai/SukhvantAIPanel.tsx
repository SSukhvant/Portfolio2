...
import { X, Send, RefreshCw, ArrowUpRight, ShieldCheck } from 'lucide-react';
...
export const SukhvantAIPanel: React.FC<SukhvantAIPanelProps> = ({ isOpen, onClose, onNavigateSection, onOpenResume, theme = 'dark' }) => {
  ...
  useEffect(() => {
    if (isOpen && messages.length === 0) {
      setMessages([{ id: 'welcome-msg', role: 'assistant', content: "Hi, I'm Sukhvant AI.\n\nAsk me anything about Sukhvant — his experience, projects, technical skills, AI work, or how he approaches building software.", timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }), sources: ["About", "Projects", "Experience", "Skills", "AI Engineering"] }]);
    }
  }, [isOpen]);
  ...
  const handleSendMessage = async (userPrompt: string) => {
    ...
    const userMessage: ChatMessage = { id: `user-${Date.now()}`, role: 'user', content: prompt, timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) };
    ...
    try {
      ...
      const assistantMessage: ChatMessage = { id: `assistant-${Date.now()}`, role: 'assistant', content: responsePayload.reply, timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }), sources: responsePayload.sources };
      setMessages((prev) => [...prev, assistantMessage]);
      setAiState('response');
      setTimeout(() => setAiState('idle'), 1200);
    } catch (_err) {
      setMessages((prev) => [...prev, { id: `error-${Date.now()}`, role: 'assistant', content: "I ran into a temporary issue retrieving that record. Please feel free to reach out directly through the contact section.", timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) }]);
      setAiState('idle');
    } finally {
  ...
  const handleSourceClick = (sourceName: string) => {
    ...
    onClose();
    if (lower.includes('project') || lower.includes('invoice') || lower.includes('saas') || lower.includes('careerloom')) onNavigateSection('projects');
    else if (lower.includes('experience') || lower.includes('brownfleet') || lower.includes('oscarblack')) onNavigateSection('experience');
    else if (lower.includes('skill')) onNavigateSection('skills');
    else if (lower.includes('ai') || lower.includes('mcp')) onNavigateSection('ai-engineering');
    else if (lower.includes('contact')) onNavigateSection('contact');
    else onNavigateSection('about');
  };
            ...
            <div className={`flex-1 overflow-y-auto p-4 sm:p-5 space-y-4 text-xs sm:text-sm ${theme === 'dark' ? 'bg-[#000000]' : 'bg-[#FAFAFA]'}`}>
              {messages.map((msg) => {
                ...
                return <div key={msg.id} className={`flex gap-3 ${isUser ? 'justify-end' : 'justify-start'}`}>
                  {!isUser && <div className="shrink-0 mt-1"><AIOrb size="sm" state="idle" /></div>}
                  <div className={`max-w-[85%] rounded-xl p-3.5 sm:p-4 leading-relaxed ${isUser ? (theme === 'dark' ? 'bg-[#0E0E0E] border border-white/15 text-white' : 'bg-[#EAEAEA] border border-[#D0D0D0] text-[#111111]') : (theme === 'dark' ? 'bg-[#060606] border border-[#151A15] text-[#E5E5E5]' : 'bg-[#FFFFFF] border border-[#E5E7E5] text-[#111111]')}`}>
                    <div className="flex items-center justify-between gap-4 mb-1.5 text-[10px] text-[#8A8A8A] font-mono-tech"><span className={isUser ? (theme === 'dark' ? 'text-white' : 'text-[#111111]') : 'text-[#00FF66] font-semibold'}>{isUser ? 'recruiter@workstation' : '✦ sukhvant-ai'}</span><span>{msg.timestamp}</span></div>
                    <div className="whitespace-pre-wrap font-sans text-xs sm:text-[13px] leading-relaxed font-light">{msg.content}</div>
                    {msg.sources && msg.sources.length > 0 && <div className="mt-3 pt-2.5 border-t border-white/10 font-mono-tech text-[11px]"><div className="text-[#8A8A8A] mb-1.5 flex items-center gap-1"><ShieldCheck className="w-3 h-3 text-[#00FF66]" /><span>Verified Evidence & Profile Records:</span></div><div className="flex flex-wrap gap-1.5">{msg.sources.map((src, sIdx) => <button key={sIdx} id={`ai-src-btn-${sIdx}`} onClick={() => handleSourceClick(src)} className={`px-2 py-0.5 rounded text-[10px] transition-colors flex items-center gap-1 font-mono ${theme === 'dark' ? 'bg-[#0D0D0D] hover:bg-[#151515] text-[#A3A3A3] hover:text-[#00FF66] border border-white/10 hover:border-[#00FF66]/30' : 'bg-[#F0F0F0] hover:bg-[#E5E5E5] text-[#555555] hover:text-[#00A84F] border border-[#E0E0E0]'}`}><span>→ {src}</span><ArrowUpRight className="w-2.5 h-2.5" /></button>)}</div></div>}
                  </div>
                </div>;
              })}
